...
AuthService::AuthService(userver::storages::postgres::ClusterPtr cluster)
    : cluster_(std::move(cluster)) {}

std::optional<std::string> AuthService::registerUser(const std::string& email,
...
std::optional<std::string> AuthService::loginUser(const std::string& email,
                                                  const std::string& password) {
    if (email.empty() || password.empty() || isBlank(email) || isBlank(password)) {
        return std::nullopt;
    }

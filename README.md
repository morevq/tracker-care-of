...
CLI приложение требует локальной сборки с помощью CMake. userver для CLI не нужен — он зависит только от cpr/nlohmann_json/fmt из vcpkg, поэтому собирается нативно на Windows. API-сервер на userver на Windows нативно не собирается (это сознательное ограничение фреймворка) — на Windows CMake автоматически выключает опцию `BUILD_API`, и собирается только `tracker_cli`. Для полной сборки (API + CLI) используйте WSL/Linux.

### Требования для сборки

- CMake 3.25+
- vcpkg (для управления зависимостями)
- Компилятор с поддержкой C++20 (на Windows — MSVC из Visual Studio 2022)
...
2. **Настройте переменную окружения** `VCPKG_ROOT` (нужно делать в каждом новом окне PowerShell):
   ```powershell
...
6. **Запустите CLI**:
   ```powershell
   .\Release\tracker_cli.exe
   ```

### Сборка API на WSL/Linux

На Linux/WSL опция `BUILD_API` включена по умолчанию — конфигурация и сборка делается так же, как выше, и собираются обе цели (`tracker_cli` и `tracker_api`). При желании можно явно управлять флагом:

```bash
cmake .. -DCMAKE_TOOLCHAIN_FILE="$VCPKG_ROOT/scripts/buildsystems/vcpkg.cmake" -DBUILD_API=ON
cmake --build . --target tracker_api
```
